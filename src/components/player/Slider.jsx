...
export function Slider(props) {
  ...
  return (
    <div
      {...groupProps}
      className="absolute inset-x-0 flex items-center flex-auto gap-6 bottom-full touch-none md:relative"
    >
      {props.label && (
      ...
      <div
        ...
        ref={trackRef}
        className="relative w-full bg-slate-700 md:rounded-full"
      >
        <div
          className={clsx(
            ...
            isFocusVisible || state.isThumbDragging(0)
              ? 'bg-pink-600'
              : 'bg-gradient-to-r from-indigo-500 via-[#92FEFC] to-[#FE90FE]'
          )}
          style={{
            width:
      ...
      <div className="items-center hidden gap-2 md:flex">
        ...
        <span
          className={clsx(
            'hidden rounded-md px-1 py-0.5 font-mono text-sm leading-6 text-slate-300 md:block',
            state.getThumbMaxValue(0) === 0 && 'opacity-0'
          )}
        >
